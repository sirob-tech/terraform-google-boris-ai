...
#
# The endpoint is idempotent: re-sending the same organization and values is a
# no-op, and a different organization for the same customer is rejected with 409.
# The trigger below re-fires the PUT only when a registered value changes, so an
# unchanged re-apply does not call the endpoint again.
#
# terraform destroy cannot call the DELETE endpoint (authenticated/team-only by
# design) — offboarding stays a two-step flow (destroy + team DELETE).
